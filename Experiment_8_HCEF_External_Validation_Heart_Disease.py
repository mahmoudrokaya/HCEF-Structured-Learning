...
Experiment 8:
Cross-domain external validation on the UCI Heart Disease Dataset.

Input folder:
D://47//471//New Papers//Paper 3 IJOCTA//Sub//Data//UCI Heart Disease Data

Output:
D://47//471//New Papers//Paper 3 IJOCTA//Sub//New_3_Experiments//Experiment_8_HCEF_External_Validation_Heart_Disease
"""
...
DATA_DIR = Path(r"D:\47\471\New Papers\Paper 3 IJOCTA\Sub\Data\UCI Heart Disease Data")
RESULTS_ROOT = Path(r"D:\47\471\New Papers\Paper 3 IJOCTA\Sub\New_3_Experiments")

EXP_DIR = RESULTS_ROOT / "Experiment_8_HCEF_External_Validation_Heart_Disease"
...
def find_dataset_file(data_dir):
    ...
    preferred_keywords = ["heart", "processed", "cleveland", "hungarian", "switzerland", "va", "data", "uci"]
    ...
    return sorted(candidates, key=score_file, reverse=True)[0]
...
def prepare_heart_dataframe(df):
    """
    Handles common UCI Heart Disease formats:
    - named columns with target/num
    - raw processed.cleveland.data style with 14 columns:
...
def detect_target_column(df):
    candidates = [
        "target", "Target", "num", "Num", "class", "Class",
        "label", "Label", "condition", "Condition", "heart_disease"
    ]
    ...
    for c in df.columns:
        lower = str(c).lower()
        if "target" in lower or lower == "num" or "class" in lower or "label" in lower:
            return c
...
def preprocess_features(df, target_col):
    ...
    if y_numeric.notna().mean() >= 0.90:
        # UCI target usually: 0 = no disease, 1-4 = disease
        y = (y_numeric.fillna(0).astype(float) > 0).astype(int).values
        target_classes = ["no_disease", "disease"]
    else:
        encoder = LabelEncoder()
        y = encoder.fit_transform(y_raw.astype(str))
        target_classes = list(map(str, encoder.classes_))
    ...
    for c in X_raw.columns:
        ...
        if numeric_ratio >= 0.80:
            ...
            enc = LabelEncoder()
            filled = s.astype(str).replace("nan", "MISSING_VALUE").fillna("MISSING_VALUE")
    ...
    return X, y.astype(int), target_classes, pd.DataFrame(encoding_rows)
...
print("=" * 90)
print("Experiment 8: HCE-F External Validation - UCI Heart Disease")
print("=" * 90)
print(f"Device: {DEVICE}")

dataset_file = find_dataset_file(DATA_DIR)
df_raw = load_dataset(dataset_file)
df = prepare_heart_dataframe(df_raw)

target_col = detect_target_column(df)

X, y, target_classes, encoding_report = preprocess_features(df, target_col)

if len(np.unique(y)) != 2:
    raise ValueError(f"Expected binary target after conversion, detected classes: {np.unique(y)}")

encoding_report.to_csv(TABLES_DIR / "encoding_report.csv", index=False)

processed_df = X.copy()
processed_df["target"] = y
processed_df.to_csv(PROCESSED_DIR / "heart_disease_processed.csv", index=False)
...
target_counts["class_label"] = target_counts["encoded_class"].apply(lambda i: target_classes[i])
...
plt.xlabel("Class")
plt.ylabel("Count")
plt.title("UCI Heart Disease Target Distribution")
...
for name, model in baselines.items():
    ...
    row = {"model": name, "training_time_seconds": float(training_time)}
    row.update(metrics)
...
model_path = MODELS_DIR / "best_hcef_heart_disease.pt"
...
hcef_row = {
    "model": "HCE-F_Tabular_Heart_Disease",
...
pd.DataFrame({
    ...
    "model": "HCE-F_Tabular_Heart_Disease"
}).to_csv(PRED_DIR / "predictions_HCE-F_Tabular_Heart_Disease.csv", index=False)
...
plt.xlabel("ROC-AUC")
plt.ylabel("Model")
plt.title("Heart Disease External Validation - ROC-AUC")
...
plt.xlabel("F1-score")
plt.ylabel("Model")
plt.title("Heart Disease External Validation - F1-score")
plt.gca().invert_yaxis()
plt.grid(axis="x", alpha=0.3)
...
plt.xticks([0, 1], target_classes)
plt.yticks([0, 1], target_classes)
plt.xlabel("Predicted")
plt.ylabel("Observed")
plt.title("HCE-F Confusion Matrix - Heart Disease")
...
plt.title("HCE-F ROC Curve - Heart Disease")
plt.legend()
plt.grid(alpha=0.3)
...
plt.title("HCE-F Precision-Recall Curve - Heart Disease")
plt.legend()
plt.grid(alpha=0.3)
...
plt.xlabel("Epoch")
plt.ylabel("Loss")
plt.title("HCE-F Training Dynamics - Heart Disease")
plt.legend()
plt.grid(alpha=0.3)
...
plt.xlabel("Epoch")
plt.ylabel("Score")
plt.title("HCE-F Validation Dynamics - Heart Disease")
plt.legend()
plt.grid(alpha=0.3)
...
summary = {
    "experiment": "Experiment_8_HCEF_External_Validation_Heart_Disease",
    "status": "completed",
...
with open(REPORTS_DIR / "experiment_8_summary.json", "w", encoding="utf-8") as f:
...
with open(REPORTS_DIR / "METHODS_TEXT_Experiment_8_HCEF_External_Validation_Heart_Disease.md", "w", encoding="utf-8") as f:
...
with open(REPORTS_DIR / "README_Experiment_8_HCEF_External_Validation_Heart_Disease.md", "w", encoding="utf-8") as f:
    f.write(readme_text)
